...
static void insertion_sort(void *_begin,
                           void *_end,
                           size_t size,
                           cmp_func_t cmp_func)
{
...
static void unguarded_insertion_sort(void *_begin,
                                     void *_end,
                                     size_t size,
                                     cmp_func_t cmp_func)
{
...
bool partial_insertion_sort(void *_begin,
                            void *_end,
                            size_t size,
                            cmp_func_t cmp_func)
{
...
bool partition_right_branchless(void *_begin,
                                void *_end,
                                size_t size,
                                cmp_func_t cmp_func,
                                char **ret_pivot)
{
    char *begin = (char *) _begin;
    char *end = (char *) _end;
    char *pivot = kmalloc(size, GFP_KERNEL);
    ...
    if (!already_partitioned) {
        ...
        char offsets_l_storage[BLOCK_SIZE] __aligned(BLOCK_SIZE);
        char offsets_r_storage[BLOCK_SIZE] __aligned(BLOCK_SIZE);
        ...
        if (num_l) {
            offsets_l += start_l;
            while (num_l--)
                do_swap(offsets_l_base + idx(offsets_l[num_l]),
                        (last -= idx(1)), size, 0);
            first = last;
        }
        if (num_r) {
...
bool partition_right(void *_begin,
                     void *_end,
                     size_t size,
                     cmp_func_t cmp_func,
                     char **ret_pivot)
